...
print("What should the badge text be?")

guard let badge = readLine(), !badge.isEmpty else {
	print("Empty badge text")
	exit(1)
}
...
let badgedIcon = NSImage(size: iconImage.size, flipped: true) { rect -> Bool in 
	...
	addBadge(badge, imageSize: iconImage.size, with: .white)
